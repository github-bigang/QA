...
#         self.titleIndexFile=param[4]
    ...
    def createIndex(self, qFile, indexFile):
        '''main of the program, creates the index'''
        self.collectionFile=qFile
...
#         self.getStopwords()
        ...
        with open(self.collectionFile,'r') as data:
            for line in data:
                items = re.split("\t", line.strip())
                qid = items[0]
                sentence1 = items[1].lower()
#                 sentence2 = items[3].lower()
                terms1=self.getTerms(sentence1)
#                 terms2=self.getTerms(sentence2)
            
                self.numDocuments+=1
            
                #build the index for the current page
                termdictPage={}
                for position, term in enumerate(terms1):
                    try:
                        termdictPage[term][1].append(position)
                    except:
                        termdictPage[term]=[qid, array('I',[position])]
