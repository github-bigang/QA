...
def evaluate(dataStream, valid_graph, sess, outpath=None, label_vocab=None, mode='prediction',char_vocab=None, POS_vocab=None, NER_vocab=None, test_mode=False):
	...
	for batch_index in xrange(dataStream.get_num_batch()):
		...
		#kazi: reading qid_batch
		(qid_batch, label_batch, sent1_batch, sent2_batch, label_id_batch, word_idx_1_batch, word_idx_2_batch, 
...
# 		correct_tags += sess.run(valid_graph.get_eval_correct(), feed_dict=feed_dict)
		if outpath is not None:
			if mode =='prediction':
				...
				#kazi: printing probability as well as qid
				probs = sess.run(valid_graph.get_prob(), feed_dict=feed_dict)
				for i in xrange(len(label_batch)):
					outline = label_batch[i] + "\t" + label_vocab.getWord(predictions[i]) + "\t" + output_probs(probs[i], label_vocab) + "\t" + sent1_batch[i] + "\t" + sent2_batch[i] + "\t" + qid_batch[i] + "\n"
	...
	if not test_mode:
		os.system('./scripts/score.sh %s' % (outpath))
		accuracy = os.popen('tail -1 ' + outpath + '.out' + '| awk \'NF>1{print $NF}\'')
		#accuracy = correct_tags / total_tags * 100
		return float(accuracy.read())
	else:
		return 0
...
#						  train_graph.get_in_passage_chars(): char_matrix_idx_2_batch, 
			...
			if (step + 1) % trainDataStream.get_num_batch() == 0 or (step + 1) == max_steps:
				...
				#kazi: intermediate evaluation on validation set
				validation_label_vocab = Vocab(FLAGS.model_prefix + ".label_vocab", fileformat='txt2')
				accuracy = evaluate(devDataStream, valid_graph, sess, outpath=FLAGS.out_path, label_vocab = validation_label_vocab, char_vocab=char_vocab, POS_vocab=POS_vocab, NER_vocab=NER_vocab)
				if accuracy > best_accuracy:
					best_accuracy = accuracy
					saver.save(sess, best_path)
					print("Current accuracy is %.2f -- Model saved" % accuracy)
				else:
					print("Current accuracy is %.2f" % accuracy)
					
	# kazi: decoding on test set at each step
	print("Best accuracy on dev set is %.2f" % best_accuracy)
	"""
	#kazi: Not decoding on the test set here. 
	# decoding
...
if __name__ == '__main__':
	...
	#kazi: test prediction output file (intermediate)
